...
const Home = () => {
  ...
  const fetchManagers = () => {
    axios.get(`${import.meta.env.VITE_API_URL}/auth/admin_records`).then((res) => {
      if (res.data.Status) {
  ...
  const fetchEmployees = () => {
    axios.get(`${import.meta.env.VITE_API_URL}/employee`).then((res) => {
      if (res.data.Status) setEmployeeTotal(res.data.Result.length);
    });
  };

  const fetchCouriers = () => {
    axios.get(`${import.meta.env.VITE_API_URL}/courier/count`).then((res) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const url = editing
      ? `${import.meta.env.VITE_API_URL}/auth/edit_manager/${editing}`
      : `${import.meta.env.VITE_API_URL}/auth/add_manager`;
  ...
  const handleDelete = (id) => {
    ...
    axios.delete(`${import.meta.env.VITE_API_URL}/auth/delete_manager/${id}`).then((res) => {
      if (res.data.Status) {
